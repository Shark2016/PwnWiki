...
![](images/堆漏洞利用/heap_overflow1.png)

![](images/堆漏洞利用/heap_overflow2.png)



> 思考：为什么chunk_n2的prev_size和size不合法也可以控制RIP
> 解答：这个案例比较特殊，结构体包含函数指针，而在调用指针利用时，也没有多余的malloc和free来进行检查，所以不需要特别构造prev_size和size就可以利用成功。

**堆溢出实例 - #2**

![](images/堆漏洞利用/heap_overflow3.png)

![](images/堆漏洞利用/heap_overflow4.png)



![](images/堆漏洞利用/heap_overflow5.png)

![](images/堆漏洞利用/heap_overflow6.png)



> 思考：为什么n2的prev_size不合法也可以正常malloc
> 解答：prev_size用于在free时用来找前一个chunk地址，这里并没有调用free，所以可以正常malloc



### 堆溢出利用 - fastbin attack

> 思考：上面例子中为何是write-anything-somewhere，而不是write-anything-anywhere?(暂不考虑tcache)
> 解答：因为malloc时会对fastbin chunk做检查，检查size是否合法

![](images/堆漏洞利用/fastbin_check1.png)
...
![](images/堆漏洞利用/fastbin_check2.png)
