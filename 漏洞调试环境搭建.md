...
elf = ELF('level3')
# io = remote('127.0.0.1',10001)      # 远程攻击
io = process('./level3')              # 本地测试
...
io = remote("127.0.0.1",10001)  #远程连接
io.close()                  #关闭连接
io.send(data)               #发送数据
...
print hex(elf.symbols['write'])    # 函数地址0x401680
print hex(elf.got['write'])       # GOT表的地址0x60b070
print hex(elf.plt['write'])        # PLT的地址0x401680
print hex(elf.search('/bin/sh').next()) # 字符串/bin/sh的地址
# 或者也可以这样获取
context.binary = "./pwn"
elf = context.binary
...
调试x86或x64下的简单缓冲区溢出，对glibc等版本基本无要求，最简单的方式是使用虚拟机安装Ubuntu即可。但涉及到复杂的堆漏洞利用，不同的libc版本其堆管理实现细节不同，通过虚拟机就不是很方便，这时候用docker或许是更便利的选择。
...
具体参考：https://github.com/Escapingbug/ancypwn

特点：基于18.04、18.10等Ubuntu，调试不同的libc时相应的用不同的版本镜像就可以，支持gdb远程调试，调试方便
...
这时候往host机器的~/pwn目录下放上你的二进制，就可以直接在容器的/pwn里看到了
...
> 技巧：vim可以通过编辑~/.vimrc配置文件实现一键运行当前exploit脚本，而不需要先退出vim再通过python exploit.py执行，可以提高效率：
...
首先先在左边窗口运行目标程序，熟悉程序交互逻辑，处理交互输入输出；熟悉的同时，在右边窗口将交互的输入输出通过python实现接口函数。
...
在实际ctf比赛中，往往提供一个二进制目标文件和一个运行该二进制的服务器端口。

我们平时自己练习的情况下，想要将某个交互式的二进制程序绑定到服务器的某个端口上，可以使用socat这个工具来完成，命令如下：
...
现在如果是使用pwntool或zio实现的exp的话，只要把`p = process('./level1')`换成`p = remote('127.0.0.1',10001)`即可。
